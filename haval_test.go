...
var havalEmptyVectors = map[string]string{
...
var havalABCVectors = map[string]string{
...
// Long alphanumeric string from the PHP HAVAL test suite (note: N is missing after M).
var havalLongInput = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMOPQRSTUVWXYZ0123456789" +
	"abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMOPQRSTUVWXYZ0123456789" +
	"abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMOPQRSTUVWXYZ0123456789"

var havalLongVectors = map[string]string{
...
func TestHavalEmpty(t *testing.T) {
	for name, want := range havalEmptyVectors {
		h, err := New(name)
		if err != nil {
...
func TestHavalABC(t *testing.T) {
	for name, want := range havalABCVectors {
		h, err := New(name)
		if err != nil {
			t.Fatalf("New(%q): %v", name, err)
		}
		...
		got := hex.EncodeToString(h.Sum(nil))
		if got != want {
			t.Errorf("%s(\"abc\") = %s, want %s", name, got, want)
		}
	}
}

func TestHavalLong(t *testing.T) {
	for name, want := range havalLongVectors {
		h, err := New(name)
		if err != nil {
			t.Fatalf("New(%q): %v", name, err)
		}
		h.Write([]byte(havalLongInput))
		got := hex.EncodeToString(h.Sum(nil))
		if got != want {
			t.Errorf("%s(long) = %s, want %s", name, got, want)
		}
	}
}

func TestHavalClone(t *testing.T) {
	for name, wantABC := range havalABCVectors {
		h, err := New(name)
		if err != nil {
			t.Fatal(err)
		}
		...
		got := hex.EncodeToString(h.Sum(nil))
		if got != wantABC {
			t.Errorf("clone %s: original got %s, want %s", name, got, wantABC)
		}
		...
		if got2 == wantABC {
			t.Errorf("clone %s: cloned should differ from abc hash", name)
		}
	}
}

func TestHavalContinueAfterSum(t *testing.T) {
	for name, want := range havalABCVectors {
		h, err := New(name)
		if err != nil {
			t.Fatal(err)
		...
		h.Write([]byte("a"))
		_ = h.Sum(nil) // intermediate hash
		h.Write([]byte("bc"))
		got := hex.EncodeToString(h.Sum(nil))
		if got != want {
			t.Errorf("continue-after-sum %s: got %s, want %s", name, got, want)
		}
	}
}
